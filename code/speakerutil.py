...
def speakermap(dirname):
	speakers, skip = dict(), 12
	for i, line in enumerate(open(os.path.join(dirname, 'SPEAKERS.TXT'))):
		if i < skip: continue
		speakers[int(line[: 4])] = i - skip
	return speakers

def train(model, sess, config, dirname, feed, speakermap):
		...
		for filename in filenames:
			if filename.endswith('.txt'):
				for line in open(os.path.join(dirpath, filename)):
					labels = [speakermap[int(line.split(' ', 1)[0].split('-')[0])]]
...
def test(model, sess, config, dirname, feed, speakermap):
	features, labelslen, labelsind, labelsval = list(), list(), list(), list()
