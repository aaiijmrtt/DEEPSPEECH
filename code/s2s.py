	...
	with tf.variable_scope(scope), tf.name_scope(scope):
		...
		with tf.variable_scope('outputs'), tf.name_scope('outputs'):
			if config.get(scope, 'decoder_type') == 'rnn_decoder':
				outputs, state = getattr(tf.nn.seq2seq, config.get(scope, 'decoder_type'))(model['%s_inputs' %scope], model['%s_init' %scope], model['%s_rnn' %scope])
			elif config.get(scope, 'decoder_type') == 'attention_decoder':
				outputs, state = getattr(tf.nn.seq2seq, config.get(scope, 'decoder_type'))(model['%s_inputs' %scope], model['%s_init' %scope], tf.transpose(model['%s_outputs' %connect], [1, 0, 2]), model['%s_rnn' %scope])
