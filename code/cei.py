...
def cei(model, config, scope, connect, threshold = 1e-5):
	with tf.variable_scope(scope), tf.name_scope(scope):
		with tf.variable_scope('inputs'), tf.name_scope('inputs'):
			...
			model['%s_inputs' %scope] = tf.clip_by_value(tf.nn.softmax(tf.reduce_sum(model['%s_outputs' %connect], 0)), threshold, 1. - threshold, name = '%s_inputs' %scope)
		...
		with tf.variable_scope('labels'), tf.name_scope('labels'):
			...
			model['%s_labels' %scope] = tf.sparse_to_dense(model['%s_labels_ind' %scope], [model['%s_in0length' %scope], model['%s_in1length' %scope]], model['%s_labels_val' %scope], name = '%s_labels' %scope)
