...
def ced(model, config, scope, connect):
	with tf.variable_scope(scope), tf.name_scope(scope):
		...
		with tf.variable_scope('loss'), tf.name_scope('loss'):
			model['%s_loss' %scope] = tf.reduce_sum(tf.where(tf.less(model['%s_labels' %scope], tf.zeros([model['%s_in0length' %scope], model['%s_maxin2length' %scope], model['%s_maxin2length' %scope]], tf.float32)), tf.zeros([model['%s_in0length' %scope], model['%s_maxin2length' %scope], model['%s_maxin2length' %scope]], tf.float32), -tf.add(tf.multiply(model['%s_labels' %scope], tf.log(tf.add(model['%s_inputs' %scope], 1e-10))), tf.multiply(tf.subtract(1., model['%s_labels' %scope]), tf.log(tf.add(tf.subtract(1., model['%s_inputs' %scope]), 1e-10))))), name = '%s_loss' %scope)
